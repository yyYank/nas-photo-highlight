...
describe('smoothFrameScores', () => {
  it('近傍平均との差で total を平滑化する', () => {
    const smoothed = smoothFrameScores([frame(0, 0), frame(1, 1), frame(2, 0)], 1)
...
describe('detectPeakFrames', () => {
  it('threshold を超える局所ピークを拾う', () => {
    const peaks = detectPeakFrames([frame(0, 0.1), frame(1, 0.8), frame(2, 0.2)], 0.5)
...
describe('mergeNearbyPeaks', () => {
  it('近いピークは高い方へマージする', () => {
    const peaks = mergeNearbyPeaks([frame(1, 0.7), frame(2, 0.9), frame(8, 0.6)], 3)
...
describe('scoreSegment', () => {
  it('最大値と上位平均の合成で区間スコアを返す', () => {
    expect(scoreSegment([frame(0, 0.2), frame(1, 0.6), frame(2, 0.8)])).toBeCloseTo(0.6933333333, 5)
  })
})
