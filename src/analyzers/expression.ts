...
export function weightFaces(faces: FaceDetection[]) {
  ...
  const rawWeights = faces.map((face) =>
    Math.max(0.01, face.faceSize * 0.7 + face.detectionConfidence * 0.3)
  )
...
function selectPrimaryFaces(faces: FaceDetection[], limit = 2) {
  return [...faces].sort((a, b) => b.faceSize - a.faceSize).slice(0, limit)
}
...
export function calculateExpressionScore(faces: FaceDetection[]): number {
  ...
  const penalty = primaryFaces.reduce(
    (sum, face, index) =>
      sum +
